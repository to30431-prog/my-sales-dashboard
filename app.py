...
# --- 🔥 數據載入引擎 (記憶體優化版) ---
@st.cache_data(show_spinner="🚀 正在全機掃描數據...", max_entries=1)
def load_data_final():
    try:
        ...
        if df is None: return None, "找不到資料檔"
        ...
        def extract_smart_code(text):
            ...
            return f"{match.group(1)}{match.group(2)}" if match else str(text).strip()[:5]
        
        # 尋找 IT_NO 欄位
        code_col = next((c for c in df.columns if c.upper() in ['IT_NO', 'ITEM_NO', 'P_NO', 'CODE']), df.columns[0])
        df['產品編號'] = df[code_col].apply(extract_smart_code)
        
        name_col = next((c for c in df.columns if c.upper() in ['TITLE', 'NAME', 'C_NAME', 'PROD_NAME']), code_col)
        df['產品名稱'] = df[name_col].astype(str)
        ...
        # 讀取名片資料 (CUST.DBF) 與 業務名稱 (LABORER.DBF)
        name_map, cust_info_map = {}, {}
        ...
        if cust_path:
            try:
                ...
                for _, row in c_df.iterrows():
                    c_tel = next((str(row[c]).strip() for c in c_df.columns if c.upper() in ['TELE1', 'COMP_TEL', 'TEL1'] and str(row[c]).strip() not in ["", "nan"]), "系統無紀錄")
                    c_add = next((str(row[c]).strip() for c in c_df.columns if c.upper() in ['CARADD', 'SEND_ADDR', 'INVOADD'] and str(row[c]).strip() not in ["", "nan"]), "系統無紀錄")
                    cust_info_map[row['clean_n']] = {"電話": c_tel, "地址": c_add}
                
                cust_map = c_df.set_index('clean_k')['clean_n'].to_dict()
                df['店家名稱'] = df['CUST_KEY'].map(cust_map).fillna(df['CUST_KEY'])
            except: df['店家名稱'] = df['CUST_KEY']
...
with st.expander("📅 時間範圍 (預設:最近30天)", expanded=False):
    preset = st.selectbox("⏳ 跳轉", ["最近 30 天", "最近 7 天", "本月", "上個月", "最近 3 個月", "今年 (YTD)", "全部"])
...
if "營覽" in analysis_mode:
    ...
    st.markdown("---")
    st.markdown("### 🏆 業績榜")
    t_s, t_c = st.tabs(["👑 業務", "🏪 店家"])
    with t_s:
        sr = v_df.groupby('業務員')['金額'].sum().reset_index().sort_values('金額', ascending=False).head(10)
        fig = px.bar(sr, x='金額', y='業務員', orientation='h', color='金額', color_continuous_scale='Blues', text_auto='.2s')
...
# ==========================================
    ...
    kw = st.text_input("1️⃣ 店家關鍵字", "", placeholder="搜尋...")
    ...
    if cust_g.empty: st.warning("無紀錄"); sel = "--"
    else:
        sel_l = st.selectbox("2️⃣ 點選確認", ["--- 請選擇 ---"] + [f"{x['店家名稱']} (${x['金額']:,.0f})" for _, x in cust_g.iterrows()])
        sel = sel_l.split(' ($')[0] if sel_l != "--- 請選擇 ---" else "--"
    if sel != "--":
        info = res_info.get(sel.strip(), {"電話": "系統無紀錄", "地址": "系統無紀錄"})
        st.markdown(f"""<div style='background: linear-gradient(to right, #ffffff, #f0f9ff); padding:15px; border-radius:15px; border: 1px solid #e1f0fa; margin-bottom: 20px;'>
            <h3 style='color:#2980B9; margin-top:0;'>🏪 {sel}</h3>
            <b>📞 {info['電話']}</b><br><span style='font-size:0.9rem;'>📍 {info['地址']}</span></div>""", unsafe_allow_html=True)
        ...
        with t1:
            ...
            if sub_1y.empty: st.info("無紀錄")
                ...
                lat['價'] = (lat['金額'] / lat['數量']).fillna(0).round(0).astype(int)
                lat_map = lat.set_index('產品全名')['價'].to_dict()
                agg = sub_1y.groupby('產品全名')[['數量', '金額']].sum().reset_index().sort_values('金額', ascending=False)
                agg['參考單價'] = agg['產品全名'].map(lat_map)
        ...
        with t2:
            ...
            d_sel = st.selectbox("看明細", ["--- 請選擇 ---"] + [f"{x['日期_CN']} (單:{x['SOURNO']} / ${x['金額']:,.0f})" for _, x in og.iterrows()])
            if " (單:" in d_sel:
...
# ==========================================
    ...
    st.markdown("### 📋 歷史底價總表")
    st.caption("請先選業務或店家以防當機")
    df_1y = res_df[res_df['OUTDATE'] >= (res_df['OUTDATE'].max() - pd.DateOffset(years=1))]
    c_f1, c_f2 = st.columns(2)
    s_s = c_f1.selectbox("1️⃣ 業務", ["-- 全部 --"] + sorted(df_1y['業務員'].unique()))
    df_1y = df_1y[df_1y['業務員'] == s_s] if s_s != "-- 全部 --" else df_1y
    s_c = c_f2.selectbox("2️⃣ 店家", ["-- 全部 --"] + sorted(df_1y['店家名稱'].unique()))
    ...
    if len(agg) > 800:
        st.warning(f"共 {len(agg)} 筆，僅顯前 800 筆"); st.dataframe(agg.head(800), use_container_width=True, hide_index=True)
    else: st.dataframe(agg, use_container_width=True, hide_index=True)
...
# ==========================================
    ...
    st.markdown("### 🎯 系列產品分析")
    c1, c2 = st.columns(2)
    pre = c1.text_input("代碼 (如 BN)", "").upper().strip()
    ran = c2.text_input("號碼 (如 1-50)", "").strip()
    if pre:
        ...
        if sub.empty: st.warning("無資料")
            ...
            st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False})
            st.markdown("#### 🛍️ 追蹤產品買家")
            sel_p = st.selectbox("選產品看誰買過", ["--- 請選擇 ---"] + pr_agg['產品全名'].tolist())
            if sel_p != "--- 請選擇 ---":
...
# ==========================================
    ...
    s_list = sorted(v_df['業務員'].unique())
    sel_s = st.selectbox("👤 選擇業務員", ["--- 請選擇 ---"] + s_list)
    if sel_s != "--- 請選擇 ---":
        s_df = v_df[v_df['業務員'] == sel_s]
        c1, c2 = st.columns(2)
        c1.metric("總業績", f"${s_df['金額'].sum():,.0f}")
        c2.metric("店數", f"{s_df['店家名稱'].nunique()}")
        st.markdown("---")
        c_list = s_df.groupby('店家名稱')['金額'].sum().sort_values(ascending=False).index.tolist()
        sel_c = st.selectbox("🔍 查該員對應店家", ["--- 請選擇 ---"] + c_list)
        if sel_c != "--- 請選擇 ---":
            st.markdown(f"**{sel_s}** 賣給 **{sel_c}** 的內容：")
            t1, t2 = st.tabs(["📦 產品總計", "🧾 單筆歷史"])
            with t1:
                st.dataframe(s_df[s_df['店家名稱'] == sel_c].groupby('產品全名')[['數量', '金額']].sum().sort_values('金額', ascending=False), use_container_width=True, hide_index=True)
            with t2:
                st.dataframe(s_df[s_df['店家名稱'] == sel_c][['日期_CN', '產品全名', '數量', '金額']].sort_values('日期_CN', ascending=False), use_container_width=True, hide_index=True)