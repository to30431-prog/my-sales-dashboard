...
st.markdown("""
    ...
    .cust-radio-group > div > label {
        ...
        border: 2px solid transparent !important; cursor: pointer !important; transition: all 0.3s ease !important; 
        box-shadow: 0 4px 10px rgba(0,0,0,0.03) !important; font-weight: bold !important; color: #5D6D7E !important;
        margin-bottom: 0px !important;
    }
    .cust-radio-group > div > label:hover, .cust-radio-group > div > label[data-checked="true"] {
...
if df is not None:
    ...
    with st.expander("📅 點擊展開：調整搜尋時間範圍", expanded=False):
        ...
        # 🌟 修改：這裡將預設值改為 index=5，也就是「最近 6 個月」
